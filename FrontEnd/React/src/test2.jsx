...
async function getFreeGames() {
  const response = await fetch(
    "https://corsproxy.io/?https://www.gamerpower.com/api/giveaways?type=game&platform=pc"
  );
  const data = await response.json();
  ...
  const processedGames = await Promise.all(
    data.map(async (game) => {
      ...
      const cleanedName = name.includes("(")
        ? name.slice(0, name.indexOf("(")).trim()
        : name;

      return getSearchResults(cleanedName);
    })
  );

  const finalGames = processedGames.filter(
    (game) => Array.isArray(game) && game.length > 0
  );

  console.log("Processed free games:", finalGames);
  return finalGames;
}
...
function StorePage({ games, openGame, likedGames, toggleLike }) {
  ...
  console.log('Free games const is:', freeGames);
  ...
  return (
    ...
    >
      <div className="card" id='Home-Free-Games'>
        ...
        <div className='library-games-container'>
          {freeGames ? (
              freeGames.map((gameSet, idx) => (
                <motion.div
                  key={gameSet[0].GameID}
                  className='main-game-card'
                  whileHover={{ scale: 1.01, x: -3, y: -3 }}
                  whileTap={{ scale: 0.95 }}>
                    <img src={`https://cdn.akamai.steamstatic.com/steam/apps/${gameSet[0].GameID}/library_600x900.jpg`} alt="" />
                  </motion.div>
              ))              
            ) : (
              <p>No free games.</p>
